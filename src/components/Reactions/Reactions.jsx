...
export default function Reactions(props) {
  ...
  const [user, setUser] = useState({ userId: "admin3" });
  const [clapNumber, setClapNumber] = useState(0);

  useEffect(() => {
    let isClap = props.claps.find((clap) => clap.userId === user.userId);
    if (isClap) {
      setClap("blue");
    } else {
      setClap("");
  ...
  async function fetchNumberOfClaps() {
    let numberOfClaps = await getNumberOfClaps(props.articleId);
    console.log(numberOfClaps);
    setClapNumber(numberOfClaps);
  }
